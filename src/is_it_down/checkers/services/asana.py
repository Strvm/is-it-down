"""Provide functionality for `is_it_down.checkers.services.asana`."""
...
class AsanaStatusPageCheck(BaseCheck):
    """Represent `AsanaStatusPageCheck`."""

    check_key = "asana_status_page"
    endpoint_key = "https://status.asana.com/api/v2/status.json"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.4
...
class AsanaApiAuthCheck(BaseCheck):
    """Represent `AsanaApiAuthCheck`."""

    check_key = "asana_api_auth"
    endpoint_key = "https://app.asana.com/api/1.0/users/me"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.35
    ...
    async def run(self, client: httpx.AsyncClient) -> CheckResult:
        ...
        if response.status_code in {401, 403}:
            ...
            if payload is None:
                ...
                metadata["error_payload_present"] = isinstance(errors, list)
...
class AsanaHomepageCheck(BaseCheck):
    """Represent `AsanaHomepageCheck`."""

    check_key = "asana_homepage"
    endpoint_key = "https://asana.com/"
    interval_seconds = 60
    timeout_seconds = 5.0
...
class AsanaServiceChecker(BaseServiceChecker):
    """Represent `AsanaServiceChecker`."""

"""Provide functionality for `is_it_down.checkers.services.figma`."""
...
class FigmaStatusPageCheck(BaseCheck):
    """Represent `FigmaStatusPageCheck`."""

    check_key = "figma_status_page"
    endpoint_key = "https://status.figma.com/api/v2/status.json"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.4
...
class FigmaApiAuthCheck(BaseCheck):
    """Represent `FigmaApiAuthCheck`."""

    check_key = "figma_api_auth"
    endpoint_key = "https://api.figma.com/v1/me"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.35
        ...
        if response.status_code in {401, 403}:
            ...
            if payload is None:
                ...
                if isinstance(error_status, int):
                    metadata["error_status"] = error_status
...
class FigmaHomepageCheck(BaseCheck):
    """Represent `FigmaHomepageCheck`."""

    check_key = "figma_homepage"
    endpoint_key = "https://www.figma.com/"
    interval_seconds = 60
    timeout_seconds = 5.0
...
class FigmaServiceChecker(BaseServiceChecker):
    """Represent `FigmaServiceChecker`."""

    service_key = "figma"
    logo_url = "https://cdn.simpleicons.org/figma"
    official_uptime = "https://status.figma.com/"

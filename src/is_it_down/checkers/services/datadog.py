"""Provide functionality for `is_it_down.checkers.services.datadog`."""
...
class DatadogStatusPageCheck(BaseCheck):
    """Represent `DatadogStatusPageCheck`."""

    check_key = "datadog_status_page"
    endpoint_key = "https://status.datadoghq.com/api/v2/status.json"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.4
...
class DatadogApiValidateCheck(BaseCheck):
    """Represent `DatadogApiValidateCheck`."""

    check_key = "datadog_api_validate"
    endpoint_key = "https://api.datadoghq.com/api/v1/validate"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.35
...
class DatadogAppCheck(BaseCheck):
    """Represent `DatadogAppCheck`."""

    check_key = "datadog_app"
    endpoint_key = "https://app.datadoghq.com/"
    interval_seconds = 60
    timeout_seconds = 5.0
...
class DatadogServiceChecker(BaseServiceChecker):
    """Represent `DatadogServiceChecker`."""

    service_key = "datadog"
    logo_url = "https://cdn.simpleicons.org/datadog"
    official_uptime = "https://status.datadoghq.com/"
    ...
    def build_checks(self) -> Sequence[BaseCheck]:
        ...
        """
        return [
            DatadogStatusPageCheck(),
            DatadogApiValidateCheck(),
            DatadogAppCheck(),
        ]

"""Provide functionality for `is_it_down.checkers.services.atlassian`."""
...
class AtlassianStatusPageCheck(BaseCheck):
    """Represent `AtlassianStatusPageCheck`."""

    check_key = "atlassian_status_page"
...
class AtlassianApiAuthCheck(BaseCheck):
    """Represent `AtlassianApiAuthCheck`."""

    check_key = "atlassian_api_auth"
    endpoint_key = "https://api.atlassian.com/me"
    interval_seconds = 60
    timeout_seconds = 5.0
    weight = 0.35
    ...
    async def run(self, client: httpx.AsyncClient) -> CheckResult:
        ...
        if response.status_code in {401, 403}:
            ...
            if payload is None:
                ...
                error_message = payload.get("error_description")
...
class AtlassianHomepageCheck(BaseCheck):
    """Represent `AtlassianHomepageCheck`."""

    check_key = "atlassian_homepage"
    endpoint_key = "https://www.atlassian.com/"
    interval_seconds = 60
    timeout_seconds = 5.0
...
class AtlassianServiceChecker(BaseServiceChecker):
    """Represent `AtlassianServiceChecker`."""
    ...
    def build_checks(self) -> Sequence[BaseCheck]:
        ...
        """
        return [
            AtlassianStatusPageCheck(),
            AtlassianApiAuthCheck(),
            AtlassianHomepageCheck(),
        ]
